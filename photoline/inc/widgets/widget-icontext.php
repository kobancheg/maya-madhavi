...
class MWP_Icon_Widget extends WP_Widget {
	function __construct() {
		parent::__construct(
			'mwp_icon_widget',
			'Photoline ' . __( 'Icons', 'photoline' ),
			array(
				'classname' => 'mwp_icon_widget', 
				'description' => __( 'Custom Text Widget with Icons', 'photoline' ),
				'width' => 250,
				'height' => 350
			)
		);
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'photoline' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></p>

		<p><label><?php _e( 'Icon:', 'photoline' ); ?></label>
		<span class="custom-icon-container"><?php echo $icon_tag; ?></span>
		<a href="#" class="view-icons"><?php _e( 'View Icons', 'photoline' ); ?></a> | <a href="#" class="delete-icon"><?php _e( 'Remove', 'photoline' ); ?></a>
		...
		<p><label for="<?php echo $this->get_field_id( 'url' ); ?>"><?php _e( 'URL:', 'photoline' ); ?></label>
		...
		<p><label for="<?php echo $this->get_field_id( 'button_text' ); ?>"><?php _e( 'Button Text:', 'photoline' ); ?></label>
		...
		<p><label for="<?php echo $this->get_field_id( 'button_color' ); ?>"><?php _e( 'Icon and Button Color:', 'photoline' ); ?></label>
		<select name="<?php echo $this->get_field_name( 'button_color' ); ?>" id="<?php echo $this->get_field_id( 'button_color' ); ?>" class="widefat">
		<?php
		$options = array(
			'default' => __( 'Default', 'photoline' ),
			'blue' => __( 'Blue', 'photoline' ),
			'red' => __( 'Red', 'photoline' ),
			'green' => __( 'Green', 'photoline' ),
		);
		...
		<p><input id="<?php echo $this->get_field_id('filter'); ?>" name="<?php echo $this->get_field_name('filter'); ?>" type="checkbox" <?php checked( isset( $filter ) ? $filter : 0 ); ?> />&nbsp;<label for="<?php echo $this->get_field_id('filter'); ?>"><?php _e( 'Automatically add paragraphs', 'photoline' ); ?></label></p>
		<?php
	}
}