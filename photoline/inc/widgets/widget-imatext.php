...
class MWP_Featured_Widget extends WP_Widget {
	function __construct() {
		parent::__construct(
			'mwp_featured_widget',
			'Photoline ' . __( 'Image-Text', 'photoline' ),
			array(
				'classname' => 'mwp_featured_widget', 
				'description' => __( 'Custom Text Widget with Image', 'photoline' ),
				'width' => 250,
				'height' => 350
			)
		);
	...
	function form( $instance ) {
		...
		<p><label><?php _e( 'Image:', 'photoline' ); ?></label>
		<span class="widget-image-container"><?php echo $img_tag; ?></span>
		<a href="#" class="select-image"><?php _e( 'Select Image', 'photoline' ); ?></a> | <a href="#" class="delete-image"><?php _e( 'Remove Image', 'photoline' ); ?></a>
		...
		<p><input id="<?php echo $this->get_field_id('filter'); ?>" name="<?php echo $this->get_field_name('filter'); ?>" type="checkbox" <?php checked( isset( $filter ) ? $filter : 0 ); ?> />&nbsp;<label for="<?php echo $this->get_field_id('filter'); ?>"><?php _e( 'Automatically add paragraphs', 'photoline' ); ?></label>
		</p>
		<?php
	}
}