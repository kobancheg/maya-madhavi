 ...
 */
add_action('widgets_init', create_function('', 'register_widget("photoline_authorbox");'));

class photoline_authorbox extends WP_Widget {
    function __construct() {
	parent::__construct(
			'photoline_authorbox',
			__( 'Photoline Author Box', 'photoline' ),
        				array(
				'classname' => 'photoline_authorbox',
				'description' => __( 'Photoline theme widget to display a Author info', 'photoline' )
				)
			);
    }
...
if ( $author_description ) : ?>
	...
	<h4><?php printf( __( '%s', 'photoline' ), $author ); // the_author_meta( 'display_name' ); ?></h4>
                <p><?php echo $author_description; ?></p>
                <p><a href="<?php echo $author_url; ?>" title="<?php _e( 'View all author posts', 'photoline' ); ?>"><?php printf( __( 'View all posts by %s', 'photoline' ), $author ); ?></a></p>
...
<?php
    ...
    function form( $instance ) {
        $instance = wp_parse_args( (array) $instance, array( 'title' => __( 'Author info', 'photoline' ) ) );
